...
for event in group_data:
    if float(event['created_at']) > start_date.timestamp() and  float(event['created_at']) < end_date.timestamp():
        if event['user_id'] == '45617020':
            result[event['created_at']] = event['text']
print(result)
